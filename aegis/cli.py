...
def main(argv=None):
    ...
    s=p.add_subparsers(dest="command"); sp=s.add_parser("scan",help="Scan a project using Phase 1 static analysis."); sp.add_argument("path",type=Path)
    ...
    if a.command=="scan":
        ...
        print(json.dumps({"project":a.path.resolve().as_posix(),"files_scanned":d["files_scanned"],"findings":len(f),"output":(a.path.resolve()/".aegis").as_posix()},sort_keys=True)); return 0
    return 0
